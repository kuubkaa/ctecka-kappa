...
- **Neznámý kód má dvě různé příčiny a dvě různé odpovědi.** Zboží, které nikdo nepojmenoval → pojmenovat (nový `Product`). Zboží, které v katalogu **už je pod jiným kódem** → přiřadit (`Alias`, `linkAndCount`). Vypsat název podruhé by založilo druhé zboží se stejným názvem — dva stejné řádky na podepsaném protokolu jsou vada.
  - Alias je **samostatná tabulka, ne přepsání kódu produktu**. `products.code` je primární klíč a `items` je na něm klíčované, takže přečíslování produktu by odpojilo všechny napočítané kusy. A tabulka se čte znovu — vrátila by původní kód a přiřazení tiše zrušila. (Pokryto testem „re-reading the sheet does not undo a link".)
  - `findProduct` se ptá **nejdřív katalogu, pak aliasů**: když si tabulka mezitím ten kód nárokovala pro skutečné zboží, vyhrává tabulka — je to seznam, který uživatel udržuje schválně.
  - **Aliasy musí být v záloze** (`FORMAT_VERSION` 3, u starších souborů chybí a čte se `?? []`). Ověřeno mutací: bez nich test obnovy zčervená. Zapomenutí zboží maže i jeho aliasy (`forgetProduct`), jinak ukazují na nic.
